...
export class ObjectStorageService {
  ...
  /**
   * Upload bytes generated server-side (e.g. AI renders) into the private object
   * dir at a deterministic relative path. Returns the normalized `/objects/...`
   * path that the serving route understands.
   */
  async uploadObjectEntity(
    ...
    const fullPath = `${entityDir}${cleanRelative}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
